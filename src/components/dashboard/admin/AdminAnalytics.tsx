...
export default function AdminAnalytics({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  const handleExportFull = () => {
    ...
    const rows = [
      // Overview summary
      { Category: 'Overview', Metric: 'Total Users', Value: data.overview.totalUsers },
      { Category: 'Overview', Metric: 'Parents', Value: data.overview.parentCount },
      { Category: 'Overview', Metric: 'Nannies', Value: data.overview.nannyCount },
      { Category: 'Overview', Metric: 'Total Revenue', Value: formatCurrency(data.overview.totalRevenue) },
      { Category: 'Overview', Metric: 'Revenue This Month', Value: formatCurrency(data.overview.revenueThisMonth) },
      { Category: 'Overview', Metric: 'Avg Call Duration', Value: formatDuration(data.overview.avgCallDuration) },
      { Category: 'Overview', Metric: 'Avg Rating', Value: data.overview.avgRating.toFixed(1) },
      { Category: 'Overview', Metric: 'Retention Rate', Value: `${data.overview.retentionRate}%` },
      { Category: 'Overview', Metric: 'Conversion Rate', Value: `${data.overview.conversionRate}%` },
      // Call stats
      ...
      // Subscriptions
      { Category: 'Subscriptions', Metric: 'FREE', Value: data.overview.freeSubscriptions },
      { Category: 'Subscriptions', Metric: 'BASIC', Value: data.overview.basicSubscriptions },
      { Category: 'Subscriptions', Metric: 'PRO', Value: data.overview.proSubscriptions },
    ];
    exportToCsv(rows, 'mumaa-analytics-report', {
      Category: 'Category',
      Metric: 'Metric',
      Value: 'Value',
    });
  ...
  const handleExportRevenue = () => {
    if (!data) return;
    const rows = data.revenueData.map((r) => ({
      Period: r.period,
      Revenue: r.revenue,
    }));
    exportToCsv(rows, 'mumaa-revenue-data', {
      Period: 'Period',
      Revenue: 'Revenue (₹)',
    });
  };
              ...
              <CardContent className="p-4">
                ...
                {card.growth !== undefined && card.growth !== 0 && (
                  <div className={cn(
                    ...
                    {Math.abs(card.growth)}% vs last period
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              {data.revenueData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.revenueData}>
                    ...
                    <Bar dataKey="revenue" name="revenue" fill={COLORS.rose} radius={[6, 6, 0, 0]} maxBarSize={48} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
              ...
              {data.userGrowthData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data.userGrowthData}>
                    ...
                    <Line type="monotone" dataKey="parents" name="Parents" stroke={COLORS.rose} strokeWidth={2} dot={{ fill: COLORS.rose, strokeWidth: 0, r: 3 }} />
                    <Line type="monotone" dataKey="nannies" name="Nannies" stroke={COLORS.emerald} strokeWidth={2} dot={{ fill: COLORS.emerald, strokeWidth: 0, r: 3 }} />
                    <Line type="monotone" dataKey="total" name="Total" stroke={COLORS.blue} strokeWidth={2} strokeDasharray="5 5" dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
          ...
          <CardContent className="p-4">
            <div className="h-72">
              {data.callVolumeData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data.callVolumeData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="period" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#9ca3af' }} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#9ca3af' }} />
                    <Tooltip content={<CustomTooltip />} />
                    <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '8px' }} iconType="circle" iconSize={8} />
                    <Area type="monotone" dataKey="total" name="Total" fill="#dbeafe" stroke={COLORS.blue} strokeWidth={2} fillOpacity={0.4} />
                    <Area type="monotone" dataKey="completed" name="Completed" fill="#d1fae5" stroke={COLORS.emerald} strokeWidth={2} fillOpacity={0.4} />
                    <Area type="monotone" dataKey="cancelled" name="Cancelled" fill="#fce7f3" stroke={COLORS.rose} strokeWidth={2} fillOpacity={0.4} />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
                <div className="flex items-center justify-center h-full text-gray-400 text-sm">No call volume data</div>
              )}
            </div>
          </CardContent>
        </Card>
              ...
              {data.busiestHours.some((h) => h.calls > 0) ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.busiestHours.filter((h) => h.calls > 0)}>
                    ...
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#9ca3af' }} />
                    <Tooltip content={<CustomTooltip />} />
                    <Bar dataKey="calls" name="Calls" fill={COLORS.amber} radius={[4, 4, 0, 0]} maxBarSize={24} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
          ...
          <CardContent className="p-4">
            <div className="h-52">
              {data.subscriptionDistribution.some((d) => d.value > 0) ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={data.subscriptionDistribution} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="value" stroke="none">
          ...
          <CardContent className="p-4">
            <div className="h-52">
              {data.callDistribution.some((d) => d.value > 0) ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={data.callDistribution} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="value" stroke="none">
      ...
      <Card className="rounded-xl border-gray-200 shadow-sm">
        <CardHeader className="px-6 py-4 border-b border-gray-100 flex flex-row items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Click headers to sort</p>
          </div>
          <Button
            variant="outline"
            size="sm"
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    ...
                    <th
                      className="text-left py-3 px-4 font-medium text-gray-500 text-xs cursor-pointer hover:text-gray-700 select-none"
                      onClick={() => handleSort('name')}
                    >
                      <span className="flex items-center gap-1">Nanny <SortIcon column="name" /></span>
                    </th>
                    <th
                      className="text-center py-3 px-3 font-medium text-gray-500 text-xs cursor-pointer hover:text-gray-700 select-none"
                    ...
                    <th
                      className="text-right py-3 px-4 font-medium text-gray-500 text-xs cursor-pointer hover:text-gray-700 select-none"
                ...
                <tbody className="max-h-96 overflow-y-auto">
                  {sortedNannies.map((nanny, idx) => (
                    <tr key={nanny.id} className="border-t border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right text-xs font-medium text-gray-900">{formatCurrency(nanny.earnings)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
