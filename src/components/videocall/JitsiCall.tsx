...
export function JitsiCall({
    ...
    try {
      ...
      const api = new JitsiMeetExternalAPI('meet.jit.si', {
        ...
        configOverwrite: {
          ...
          // Disable pre-join screen for MUMAA (we handle connecting state ourselves)
      ...
      api.addEventListener(JITSI_EVENTS.VIDEO_CONFERENCE_JOINED, () => {
        if (isDisposedRef.current) return
      ...
      api.addEventListener(JITSI_EVENTS.PARTICIPANT_JOINED, () => {
        if (isDisposedRef.current) return
        try {
      ...
      api.addEventListener(JITSI_EVENTS.PARTICIPANT_LEFT, () => {
        if (isDisposedRef.current) return
        try {
          const count = api.getNumberOfParticipants()
          onParticipantsChange?.(count)
      ...
      // Audio mute changes
      api.addEventListener(JITSI_EVENTS.AUDIO_MUTED_CHANGED, (e: { muted: boolean } | null) => {
        if (isDisposedRef.current) return
        onAudioMutedChange?.(!!e?.muted)
      })

      // Video mute changes
      api.addEventListener(JITSI_EVENTS.VIDEO_MUTED_CHANGED, (e: { muted: boolean } | null) => {
        if (isDisposedRef.current) return
        onVideoMutedChange?.(!!e?.muted)
      })

      // Screen sharing changes
      api.addEventListener(JITSI_EVENTS.SCREEN_SHARE_STATUS_CHANGED, (e: { on: boolean } | null) => {
        if (isDisposedRef.current) return
        onScreenShareChange?.(!!e?.on)
      })

      // Conference left / ready to close
      api.addEventListener(JITSI_EVENTS.CONFERENCE_LEFT, () => {
        if (isDisposedRef.current) return
        handleCallEnded()
      })

      api.addEventListener(JITSI_EVENTS.READY_TO_CLOSE, () => {
        if (isDisposedRef.current) return
        handleCallEnded()
      })
  ...
  }, [roomName, userName, userEmail, userAvatar, loadJitsiScript, onCallEnd, onError, onMeetingReady])
  ...
  }, [disposeJitsi, onCallEnd])

  // Manual end call from parent
  const endCall = useCallback(() => {
    if (apiRef.current) {
      try {
        apiRef.current.executeCommand('hangup')
      } catch {
        // If hangup command fails, force dispose
        handleCallEnded()
      }
    } else {
      handleCallEnded()
  ...
  // Expose endCall method via ref-like pattern
  useEffect(() => {
    // Store endCall in a global ref so parent can call it
    const el = containerRef.current
    if (el) {
      (el as HTMLDivElement & { _mumaaEndCall?: () => void })._mumaaEndCall = endCall
    }
  }, [endCall])
